...
describe("Error Handling Constants", () => {
  ...
  it("should have meaningful error messages", () => {
    Object.entries(errorHandlingConstants).forEach(([, config]) => {
  ...
  it("should not have duplicate error codes", () => {
    const errorCodes = Object.values(errorHandlingConstants).map(
      (config) => config.defaultErrorCode
    );
  ...
  it("should have consistent message format", () => {
    Object.values(errorHandlingConstants).forEach((config) => {
      expect(config.defaultErrorMessage).toMatch(
