...
describe("getFamilyIncomeRecordsSchema validation", () => {
  ...
  it("should validate with both sort orders", () => {
    ...
    expect(getFamilyIncomeRecordsSchema.validate(validInputAsc).error).toBeUndefined();
    expect(getFamilyIncomeRecordsSchema.validate(validInputDesc).error).toBeUndefined();
  });
  ...
  describe("filters validation", () => {
    it("should fail when dateFrom is invalid", () => {
      ...
      const { error } = getFamilyIncomeRecordsSchema.validate(invalidInput);
      expect(error).toBeDefined();
      expect(error.details[0].message).toBe('"dateFrom" must be a valid ISO date');
    });
    ...
    it("should fail when dateTo is invalid", () => {
      ...
      const { error } = getFamilyIncomeRecordsSchema.validate(invalidInput);
      expect(error).toBeDefined();
      expect(error.details[0].message).toBe('"dateTo" must be a valid ISO date');
    });
    ...
    it("should fail when contributorId is invalid ObjectId", () => {
      ...
      const { error } = getFamilyIncomeRecordsSchema.validate(invalidInput);
      expect(error).toBeDefined();
      expect(error.details[0].message).toBe('"contributorId" must be a valid ObjectId');
    });
  ...
  describe("sort validation", () => {
    it("should fail when sortOrder is invalid", () => {
      ...
      const { error } = getFamilyIncomeRecordsSchema.validate(invalidInput);
      expect(error).toBeDefined();
      expect(error.details[0].message).toBe('"sortOrder" must be one of [asc, desc]');
    });
